...
public class AdmissionWebHook implements ManagementEndpoint {
    ...
    protected final AdmissionReview processRequest(Buffer body) {
        AdmissionReview reviewResponse = null;
        try {
    ...
    protected final AdmissionReview review(AdmissionReview incomingReview) {
        ...
        if (!GROUP.equals(request.getRequestKind().getGroup())) {
            ...
            response = badRequestResponse(request.getUid(), format("Group '%s' unsupported, expected '%s'", request.getRequestKind().getGroup(), GROUP));
            ...
            LOGGER.debug("Operation '{}' requested by '{}' on Resource '{}' with name '{}' (Namespace: {})",
            ...
            final Status status = factory.getValidator(request)
                    .validate(request.getOperation());
