...
import io.gravitee.gateway.services.ingress.crd.resources.service.BackendServiceReference;
import io.gravitee.gateway.services.ingress.crd.resources.service.GraviteeService;
import io.gravitee.gateway.services.ingress.crd.resources.service.ServiceEndpoint;
import io.gravitee.gateway.services.ingress.crd.resources.service.ServicePath;
...
public class CustomResourceControllerService extends AbstractService {
    ...
    private void processGraviteeServices(GraviteeServices gioServices) {
        ...
        spec.getServices()
                .stream()
                .map(serviceDescription -> buildApiDescription(gioServices, serviceEnabled, serviceDescription))
                .forEach(api -> {
    ...
    private Api buildApiDescription(GraviteeServices gioServices, boolean serviceEnabled, GraviteeService serviceDescription) {
        // TODO test type to construct the right Object (for now only API)
        Api api = new Api();
        api.setName(serviceDescription.getName());
        api.setId(buildApiId(gioServices, serviceDescription));
        ...
        proxy.setVirtualHosts(serviceDescription.getEntrypoints()
                .stream()
                .filter(v -> {
                    LOGGER.info("VirtualHost({},{}) = {}", v.getVhost(), v.getPath(), v.isEnabled());
                    return v.isEnabled();
                })
                .map(v -> {
                    LOGGER.info("VirtualHost({},{})", v.getVhost(), v.getPath());
                    return new VirtualHost(v.getVhost(), v.getPath());
    ...
    private String buildApiId(GraviteeServices gioServices, GraviteeService service) {
        return service.getName() + "." + gioServices.getMetadata().getName() + "." + gioServices.getMetadata().getName();
    }
    ...
    private void deleteGraviteeServices(GraviteeServices gioServices) {
        ...
        spec.getServices()
                .stream()
                .map(serviceEntry -> buildApiId(gioServices, serviceEntry))
                .forEach(apiId -> {
    ...
    private Set<EndpointGroup> buildEndpoints(List<ServiceEndpoint> endpoints) {
        return endpoints.stream().map(endpoint -> {
            // TODO manage all types
            EndpointGroup endpointGroup = endpoint.getConfiguration();
            endpointGroup.setName(endpoint.getName());
            boolean useHttps = endpointGroup.getHttpClientSslOptions() != null;
            BackendServiceReference backendSvcRef = endpoint.getBackendService();
    ...
    private Optional<Policy> buildPolicyFromPlugin(GraviteePluginReference pluginRef) {
        GraviteePlugin gioPlugin = this.gioPluginClient.inNamespace(pluginRef.getNamespace()).withName(pluginRef.getResource()).get();
        Optional<GraviteePluginSpec.Plugin> plugin = gioPlugin.getSpec().getPlugin(pluginRef.getName());
        if (plugin.isPresent()) {
            try {
                if ("policy".equalsIgnoreCase(plugin.get().getType())) {
                ...
                LOGGER.warn("Unable to process policy configuration for plugin reference {}", pluginRef, e);
            }
        }
